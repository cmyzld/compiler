...
void initTable()
{
	for(int i = 0; i < TSIZE; i++)
		table[i] = NULL;
	/*basic int type*/
	struct Type *intType = (struct Type *)malloc(sizeof(struct Type));
	intType->kind = BASIC;
	intType->basic = INT;

    char *r = "read";
	struct FieldList *funcRead = (struct FieldList *)malloc(sizeof(struct FieldList));
	struct Type *t1 = (struct Type *)malloc(sizeof(struct Type));
	funcRead->name = r;
	t1->kind = FUNCTION;
	t1->function.state = DEFINED;	
	t1->function.retValueType = intType;
	t1->function.params = NULL;
	funcRead->type = t1;
	insert(funcRead);

	char *w = "write";
	struct FieldList *funcWrite = (struct FieldList *)malloc(sizeof(struct FieldList));
	struct Type *t2 = (struct Type *)malloc(sizeof(struct Type));
	memcpy((void *)t2, (void *)t1, sizeof(t1));
	t2->function.params = (struct FieldList *)malloc(sizeof(struct FieldList));
	t2->function.params->type = intType;
	funcWrite->name = w;
	funcWrite->type = t2;
	insert(funcWrite);
}
